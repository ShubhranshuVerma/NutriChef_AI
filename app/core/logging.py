...
def configure_logging(level: str | None = None) -> None:
    ...
    # httpx2/httpcore2 are what LangSmith uses to send traces - one INFO line per upload.
    for noisy in ("httpx", "httpcore", "httpx2", "httpcore2", "urllib3", "chromadb",
                  "sentence_transformers", "mlflow", "google_genai", "huggingface_hub"):
        logging.getLogger(noisy).setLevel(logging.WARNING)

    # This one warns on every single call about a feature we do not use.
    logging.getLogger("google_genai.models").setLevel(logging.ERROR)

    # Some Gemini models have fixed sampling and ignore our temperature setting.
    # Worth knowing once, not three times per run.
